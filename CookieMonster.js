...
CM.Cache.RemakeBuildingsBCI = function() {
	...
	for (var i in CM.Cache.Objects) {
		...
		if (CM.Disp.minDone == -1 || (CM.Cache.Objects[i].bci < CM.Disp.minDone && Game.cookies > CM.Cache.Objects[i].bci)) CM.Disp.minDone = CM.Cache.Objects[i].bci;
	}
	CM.Disp.mid = ((CM.Disp.max - CM.Disp.min) / 2) + CM.Disp.min;
	for (var i in CM.Cache.Objects) {
		var color = '';
		if (CM.Cache.Objects[i].bci == CM.Disp.min) color = CM.Disp.colorGreen;
		else if (CM.Cache.Objects[i].bci == CM.Disp.minDone) color = CM.Disp.colorBlue;
